...
Menu Premium
Code By XyraaCode Dev 2024 - New
"""
...
class MAIN:
   ...
   def Me(self):
       self.MyRich('''[white] ╦╔╗╔╔═╗╔╦╗╔═╗╔═╗╦═╗╔═╗╔╦╗[red]|Tools   : Instagram Brute
 [white]║║║║╚═╗ ║ ╠═╣║ ╦╠╦╝╠═╣║║║[red]|Created : XyraaCode
 [white]╩╝╚╝╚═╝ ╩ ╩ ╩╚═╝╩╚═╩ ╩╩ ╩[red]|Version : 1.0[/]
''',None,'[white]> [green]BANNER[/] <')
...
[red]12. [white]Requests Perbaikan\n ''',True)
   ...
   def chs(self, assets):
       while True:
         ...
         elif x in ['09','9']:exit(os.system('xdg-open https://wa.me/+6281221523195?text='))
         elif x in ['10']:os.system('rm -rf data/IG-login.txt');self.insta()
         elif x in ['12']:
              exit(os.system('xdg-open https://wa.me/+6281221523195?text=Mas+Mau+Requests+Perbaikan+Script+Ibrut'))
...
[white] 01. Set Tema Hijau          04. Set Tema Putih\n\
 ...
 07. Set Tema Default',True)
       while True:
          x = Console(style=Tema[0]).input(f'   └──> ')
          if x in   ['01','1']:open('cat_rich.py','w').write("xyraacode = 'color(10)'")
          elif x in ['02','2']:open('cat_rich.py','w').write("xyraacode = 'color(9)'")
          elif x in ['03','3']:open('cat_rich.py','w').write("xyraacode = 'color(4)'")
          elif x in ['04','4']:open('cat_rich.py','w').write("xyraacode = 'color(7)'")
          elif x in ['05','5']:open('cat_rich.py','w').write("xyraacode = 'color(5)'")
          elif x in ['06','6']:open('cat_rich.py','w').write("xyraacode = 'color(3)'")
          elif x in ['07','7']:open('cat_rich.py','w').write("xyraacode = 'color(8)'")
...
[white]Masukan Nama File Untuk Melihat Hasil, Gunakan koma untuk pemisahan nama file',True)
   ...
   def dumps(self, cintil, typess, xyz = []):
       if 'csrftoken' not in str(cintil['cookie']):
          try:
              ...
              os.system('rm -rf data/IG-login.txt')
              exit(f'\n{P}[{K}!{P}] Csrftoken tidak tersedia, dump tidak akan berjalan: {e}')
       self.MyRich('[white]Masukan username akun instagram Pastikan (Target Memiliki Followers, Akun Bersifat Publik, Tidak Centang Biru) Ingin banyak target gunakan tanda koma sebagai pemisah contohnya [green](xyraacodedev,xyraacode)[/]',True)
   ...
   def Graphql(self, typess, userid, cokie,after):
       ...
       self.csr = 'variables={"id":"%s","first":100,"after":"%s"}'%(userid,after)
...
try:
    v = requests.get('https://raw.githubusercontent.com/ZuraKanagami/Instagram/main/version.json').json()['SCBD']['version']
    if(versi == str(v)):print(f'\n   anda berada di versi terbaru : {v}');time.sleep(1)
    else:
